...
class Network(torch.nn.Module):
    def __init__(self, netParams):
        ...
        self.convg1 = Goole_module(34, 17).to(device)  # 1,512,512 -> 64,256,256
        self.convg2 = Goole_module(68, 16).to(device)  # 1,512,512 -> 64,256,256
        self.convg3 = Goole_module(64, 8).to(device)
        self.pool = slayer.pool(2)
    ...
    def forward(self, spikeInput):
        ...
        batch = len(spikeGoogle1)
        spikeGoogle1 = spikeGoogle1.to(device)
        tempGoogle2 = self.convg2(self.slayer.psp(spikeGoogle1[0]))

        shape = list(tempGoogle2.shape)
        shape.insert(0, batch)
        spikeGoogle2 = torch.zeros(shape)
        for batch_n in range(1, batch):
            tempGoogle2 = self.convg2(self.slayer.psp(spikeGoogle1[batch_n]))
            spikeGoogle2[batch_n] = tempGoogle2


        batch = len(spikeGoogle2)
        spikeGoogle2 = spikeGoogle2.to(device)
        tempGoogle3 = self.convg3(self.slayer.psp(spikeGoogle2[0]))

        shape = list(tempGoogle3.shape)
        shape.insert(0, batch)
        spikeGoogle3 = torch.zeros(shape)
        for batch_n in range(1, batch):
            tempGoogle3 = self.convg3(self.slayer.psp(spikeGoogle2[batch_n]))
            spikeGoogle3[batch_n] = tempGoogle3

        spikeLayer = self.slayer.spike(self.pool(self.slayer.psp(spikeGoogle3.to(device))))  # 8,  8, 32
        # spikeOut = self.slayer.spike(self.fc1(self.slayer.psp(spikeLayer3)))  # 10
        spikeOut = self.slayer.spike(self.fc1(self.slayer.psp(spikeLayer)))  # 10
